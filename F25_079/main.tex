...
\usepackage{longtable}   % long tables (Pandoc supports this)
\usepackage{booktabs}    % nice tables
\usepackage{array}       % p{} column type
\usepackage{tabularx}    % flexible tables

% --- Optional but safe ---
\usepackage{float}       % figure placement (Pandoc mostly ignores)
...
\makeatletter
    \begin{titlepage}
        \includegraphics{Figures/nulogo.png}\hfill
        \includegraphics{Figures/fast.png}\\
        \begin{center} 
            {\large\bfseries \university}\\[7ex]
            \includegraphics{Figures/fastlogo.png}\\[8ex]
...
Name: Ahmed Nasir
\\[3ex]
Signature: \includegraphics{Ahmed.png}
\\[6ex]
Name: Aanish Waseem
\\[3ex]
Signature: \includegraphics{aanish.png}
\\[6ex]
Name: Muhammad Salman Amir
\\[3ex]
Signature: \includegraphics{Salman.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics{Figures/SDG 9.jpg}
...
\paragraph{Key Findings from Literature}
Recent studies have explored various approaches to securing ad-hoc networks. Majumder et al.\ \cite{ref:majumder:2025a} demonstrated that while blockchain and homomorphic encryption can reduce delay and improve detection, they incur high computational costs. Similarly, Kudva et al.\ \cite{ref:kudva:2021} achieved high packet delivery ratios using a two-level trust model, though at the cost of strict trust thresholds. Other notable contributions include Swamy et al.'s deep learning optimization \cite{ref:swamy:2024}, which offers low latency but high complexity, and Morin et al.'s lightweight cipher approach \cite{ref:morin:2025}, which efficiently manages keys but relies on a centralized manager. Approaches using clustering (Sugumaran et al. \cite{ref:Sugumaran:2025}, Agrawal et al. \cite{ref:agrawal:2023}) and double-layer blockchains (Hou et al. \cite{ref:hou:2023}) have shown promise in improving network stability and trust, though often dealing with trade-offs in scalability or architectural complexity.
\pagebreak
...
\subsection*{Use Case: Route Discovery}
\label{uc:route_discovery}

\begin{description}
    \item[Actors:] User
    \item[Summary:] The source node initiates a secure route discovery process to find the shortest path to the destination node using Dijkstra's algorithm and blockchain validation.
    \item[Pre-Conditions:] Source node is registered in the network with a valid key pair.
    \item[Post-Conditions:] All intermediate nodes have verified the route, and route information is stored in the blockchain.
    \item[Special Requirements:] None
\end{description}

\subsubsection*{Basic Flow}
\begin{enumerate}
    \item \textbf{Actor:} The Source Node initiates a route discovery request to send data to a specific destination node.
    \item \textbf{System:} The system triggers Dijkstra’s algorithm to compute the shortest and most efficient route across available nodes.
    \item \textbf{Actor:} The source node digitally signs the Route Request (RREQ) and broadcasts it to neighboring nodes.
    \item \textbf{System:} The system attaches the route and security data to the blockchain ledger.
    \item \textbf{Actor:} Source node broadcasts RREQ message.
    \item \textbf{System:} System broadcasts RREQ to all neighbor nodes in the shortest path.
    \item \textbf{System:} System checks the certificate and signature at each hop.
    \item \textbf{System:} System confirms successful route discovery once the destination node gets the RREQ and an RREP is generated back along the path.
    \item \textbf{Actor:} The source node receives the RREP.
\end{enumerate}

\subsubsection*{Alternative Flow}
\begin{enumerate}
    \item \textbf{Actor:} The source node signs the RREQ again and broadcasts it.
    \item \textbf{System:} If blockchain verification fails, the route discovery process is aborted, and a new RREQ is initiated.
\end{enumerate}

\subsection*{Use Case: Data Packet Transmission}
\label{uc:data_transmission}

\begin{description}
    \item[Actors:] Source Node, Intermediate Nodes, Destination Node
    \item[Summary:] The source node sends an encrypted and signed data packet to the destination node along the pre-established and blockchain-verified route.
    \item[Pre-Conditions:] A secure and verified route between the source and destination nodes has been discovered and logged in the blockchain.
    \item[Post-Conditions:] The data packet is successfully received and decrypted by the destination node, with the transmission record updated in the blockchain.
    \item[Special Requirements:] None
\end{description}

\subsubsection*{Basic Flow}
\begin{enumerate}
    \item \textbf{Actor:} The Source Node encrypts the data packet with the destination's public key and signs it with its private key.
    \item \textbf{System:} The system forwards the packet to the first hop in the established route.
    \item \textbf{Actor:} Each Intermediate Node receives the packet.
    \item \textbf{System:} The system at each hop verifies the source's signature and the integrity of the route by checking the blockchain.
    \item \textbf{System:} If verification is successful, the node forwards the packet to the next hop.
    \item \textbf{Actor:} The Destination Node receives the encrypted packet.
    \item \textbf{System:} The system uses the destination's private key to decrypt the packet and verifies the source's signature.
\end{enumerate}

\subsection*{Use Case: Node Registration}
\label{uc:node_registration}

\begin{description}
    \item[Actors:] New Node, Existing Network Nodes
    \item[Summary:] A new node requests to join the network, generates a key pair, and has its identity certificate validated and recorded on the blockchain by existing trusted nodes.
    \item[Pre-Conditions:] The new node has the necessary software and is within communication range of the network. The network has an established blockchain.
    \item[Post-Conditions:] The new node's public key and certificate are stored on the blockchain, allowing it to participate in secure network activities.
    \item[Special Requirements:] Access to a trusted Certificate Authority (CA) or a decentralized trust mechanism.
\end{description}

\subsubsection*{Basic Flow}
\begin{enumerate}
    \item \textbf{Actor:} A New Node generates a public-private key pair and a Certificate Signing Request (CSR).
    \item \textbf{System:} The system broadcasts a "join request" to neighboring nodes, including its public key and certificate.
    \item \textbf{Actor:} Existing nodes receive the join request.
    \item \textbf{System:} The system validates the new node's certificate against the trusted CA. A consensus protocol is initiated among existing nodes to approve the new member.
    \item \textbf{Actor:} The new node receives confirmation of successful registration.
    \item \textbf{System:} Upon reaching consensus, the system adds the new node's public key, certificate, and initial trust score to a new block on the blockchain.
\end{enumerate}

\subsubsection*{Alternative Flow}
\begin{enumerate}
    \item \textbf{Actor:} The new node is denied entry to the network.
    \item \textbf{System:} If certificate validation fails or consensus is not reached, the system rejects the registration request and logs the failed attempt.
\end{enumerate}


\subsection*{Use Case: Dynamic Route Recalculation}
\label{uc:route_recalculation}

\begin{description}
    \item[Actors:] Intermediate Node, Source Node
    \item[Summary:] When a link in an established route breaks, an intermediate node detects the failure, notifies the source, and a new secure route discovery is initiated to restore communication.
    \item[Pre-Conditions:] An active and blockchain-verified route exists, and data packets are being transmitted between a source and destination node.
    \item[Post-Conditions:] A new, secure route to the destination is established and recorded on the blockchain, or the destination is marked as unreachable if no new path can be found.
    \item[Special Requirements:] Nodes must be able to detect link failures (e.g., via missing acknowledgments).
\end{description}

\subsubsection*{Basic Flow}
\begin{enumerate}
    \item \textbf{Actor:} An Intermediate Node fails to forward a packet to the next hop in the route (e.g., link break detected).
    \item \textbf{System:} The system generates a Route Error (RERR) message and sends it back towards the source node along the path the packet came from.
    \item \textbf{Actor:} The Source Node receives the RERR message.
    \item \textbf{System:} The system invalidates the broken route in its records and initiates a new "Secure Route Discovery" process to find an alternative path to the destination.
    \item \textbf{Actor:} A new route is successfully discovered.
    \item \textbf{System:} The system updates the blockchain with the new route information, and data transmission resumes.
\end{enumerate}

\subsubsection*{Alternative Flow}
\begin{enumerate}
    \item \textbf{Actor:} The source node is notified that a new route cannot be found.
    \item \textbf{System:} If the route discovery process fails to find any valid path to the destination, the system marks the destination as unreachable and informs the source node.
\end{enumerate}

\section{Hardware and Software Requirements}
\subsection{Hardware Requirements}
\FloatBarrier
The proof-of-concept network simulation runs on a cluster of Linux PCs. Each node is configured with an Intel Core i5 processor, 8GB of RAM, and a 256GB SSD running Ubuntu 22.04 LTS. This hardware specification ensures sufficient resources to handle packet encryption, blockchain operations, and trust diary updates during the small-scale PANET simulations involving 3 to 5 nodes.

\subsection{Software Requirements}

\FloatBarrier
The system relies on a robust open-source software stack. The core operating environment is Ubuntu 22.04 LTS, serving as the host for all blockchain nodes and Python simulation scripts. The blockchain framework, along with certificate issuance and consensus mechanisms, is implemented in pure C/C++ using modern compilers (g++, clang). Cryptographic operations, including key pair generation and certificate signing, leverage standard libraries like OpenSSL. Additionally, custom C++ simulation tools are used to model the PANET environment and test various attack scenarios.
        ...
        Following are the important components of the  Security Framework
            ...
            \begin{itemize}
                ...
                \item \textbf{Trust Update (Bayesian / Beta-Bernoulli):}
                    Alpha = p × s, Beta = (1 - p) × s, p_new = (Alpha + Acks) / (s + n) where n = Acks + NAcks, s_new = s + n; Acks = NAcks = 0
            ...
            \begin{figure}[htbp]
                \centering
                \includegraphics{Figures/pan_local_trust_flowchart_colorful.png}
                ...
                \begin{itemize}
                    ...
                    \item \textbf{Consensus Mechanism:} \\
                        ...
                        \begin{enumerate}
                            ...
                            MinThresh_i = PrevMinThresh + k1 × Density Change + k2 × Network Reception Change
      \textbf{Intermediate Variables: } \\
                            Nt = TotalCertificatesIssued(t) - TotalCertificatesExpired(t)
                            Pt = Number of Neighbors received the Hello Packet
                            \item Derived Quantities : \\
                            DensityChange = (Nt - Nt-1) / (Nt-1 + 1)
                            \\
                            NetworkReceptionChange = (Pt / Nt) - PrevMinThreshold
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics{Figures/PKCertChain_Node_Registration_Flowchart.png}
                    \caption{PKCertChain}
                    \label{fig:LTDs}
                \end{figure}
            ...
            \begin{figure}[htbp]
                \centering
                \includegraphics{Figures/PKCertChain_CertRenewal-1_page-0001.jpg}
                \caption{Certification Renewal takes place when it has expired. The request will be sent to blockchain.}
            ...
            \begin{enumerate}
                ...
                \item \textbf{Consensus Mechanism: }
                    Score = Alpha × TotalTrust + Beta × Latency + Gamma × Hops + Epsilon × Bandwidth
                ...
                \textbf{Sender Packet:} \\
                The sender encrypts the message with the destination’s public key and appends its signature: \\
                Packet\_sender = Enc\_PK\_DESTINATION || Sign\_Sender(Message)
                
                \textbf{Intermediate Nodes Packet and Rolling Signatures:} \\
                At the i-th intermediate node, the received packet is: \\
                Packet\_i = Enc\_PK\_INTERMEDIATE\_i || Sign\_i(Packet\_i-1)
                ...
                \begin{figure}[htbp]
                \centering
                \includegraphics{Figures/RouteLogChain_Sender_Flowchart-2.png}
                \caption{Every node has to verify signature. If packet not received then re-transmit packet. The Destination node will verify by using Rolling Signature.}
                \label{fig:RLC}
                \end{figure}
            \FloatBarrier
            \section{Meta Blockchain}
                ...
                \begin{figure}[htbp]
                \centering
                \includegraphics{Figures/Meta_Blockchain_page-0001.jpg}
                \caption{MetaBlockchain is responsible for routing either to PKI Blockchain or Route Log Blockchain}
                \label{fig:MetaBC}
                \end{figure}
...
\begin{figure}[!h]
    \centering
    \includegraphics{Figures/blockchain.PNG} % adjust width as needed
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/SybilDFD2.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/blackhole_attack_DFD.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/wormhole_level2_DFD.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/replay_level2_DFD.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics{Figures/architecture.png} % adjust width as needed
...
\FloatBarrier
The project utilizes a comprehensive set of development tools. Development and testing are conducted on Ubuntu 22.04 LTS using C23 and C++23 standards compiled with GCC 11 or Clang 14. Concurrency is managed through standard mutexes and atomic operations, while low-level networking is handled via POSIX sockets. The blockchain architecture uses custom linked lists with hash pointers, and cryptography is provided by ECC and ECDSA algorithms. Build automation is managed by CMake.
\FloatBarrier
...
\FloatBarrier
\subsubsection{Test Campaign Plan}
The evaluation framework begins with a rigorous test campaign. This involves tracking the number of test cases developed and passed to ensure complete functional and security coverage. We also measure defect density to assess implementation reliability and maintain a traceability matrix to guarantee that every requirement is mapped to a specific test.
\FloatBarrier
\subsection{Security Effectiveness Metrics}
\FloatBarrier
\subsubsection{Security Effectiveness Metrics}
To validate security, we measure detection and mitigation rates for Sybil, Blackhole, and Wormhole attacks. We also evaluate the system's resistance to Replay attacks and track the false positive rate to ensure legitimate nodes are not penalized. The average time taken to mitigate an attack is recorded to assess the system's responsiveness.
\FloatBarrier
\clearpage
\subsection{Computational Overhead Metrics}
\FloatBarrier
\subsubsection{Computational Overhead Metrics}
The computational cost of security is assessed by measuring average processing times for PKCertChain blocks (PoW and voting), RouteLogChain blocks, and Bayesian Trust Diary updates. We also quantify the per-hop latency introduced by rolling signature verification and full packet processing (encryption/decryption). Additionally, memory and CPU overhead per node are monitored to ensure the system remains lightweight.
\FloatBarrier
\subsection{Network Performance Metrics}
\FloatBarrier
\subsubsection{Network Performance Metrics}
Network performance is evaluated through standard metrics such as Average End-to-End Delay and Packet Delivery Ratio (PDR) under both normal and attack scenarios. We specifically measure delay and response times during Sybil, Blackhole, and Wormhole attacks. Throughput, control overhead (from routing and blockchain), and bandwidth usage are also tracked. Finally, we measure the time required for the network to converge after a topology or trust change.
\FloatBarrier
...
\begin{figure}[ht]
    \centering
    \includegraphics{Figures/simulation_graph.jpg} % Replace with your file name
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_blackhole.png} % Replace with your file name
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_wormhole.png} % Replace with your file name
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_sybil.png} % Replace with your file name
...
Frequent route recalculations significantly influence network performance. Specifically, they tend to increase **End-to-End Delay** as the system must discover new optimal routes and perform blockchain validation for each change. This effect is exacerbated under high mobility. Furthermore, while necessary for security, frequent recalculations can reduce the **Packet Delivery Ratio (PDR)** if the discovery process lags behind network changes, potentially leading to dropped packets when processing resources are constrained.
...
Comparing the proposed Modified DSR against the standard DSR protocol reveals distinct trade-offs. While the standard DSR suffers from lower throughput and poor security against dead or malicious nodes, the Modified DSR achieves higher throughput and significantly better detection of network failures. Although the Modified DSR incurs higher energy utilization due to the added security layers, it provides critical Features like blockchain-based integrity that standard DSR lacks.
...
Simulation results indicate the average time costs for key security operations. Block creation for routing updates is the most intensive task, taking approximately 8ms. Encryption adds about 6ms per packet, while block verification via lightweight consensus requires 5ms. Data transmission and reception processing, including metadata encapsulation and decryption, take 3ms and 2ms respectively.
\section{Conclusion}
