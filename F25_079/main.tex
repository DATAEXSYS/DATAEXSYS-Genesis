...
% ---------- Custom Use Case Box ----------
% NOTE: Pandoc IGNORES advanced boxes → we keep it simple
...
\title{Secure Data Exchange System for Ad-hoc Networks Using Blockchain-Enhanced DSR}

\author{
Ahmed Nasir (22L-6644) \\ 
Aanish Waseem (22L-6887) \\
Muhammad Salman Amir (22L-6830)
\\[1em]
\degree\ Thesis
\\[1em]
Supervisors: Mr. Naveed, Dr. Rana Asif ur Rehman
\\ \university
}
...
\chapter*{Anti-Plagiarism Declaration}
This is to declare that the above publication was produced under the:
\begin{flushleft}
\bf Title: \fyptitle
\end{flushleft}

is the sole contribution of the author(s), and no part hereof has been reproduced as it is the basis that can be considered Plagiarism. All referenced parts have been used to argue the idea and cited properly. I/We will be responsible and liable for any consequence if a violation of this declaration is determined.
...
To tackle these challenges, the system uses a secure routing protocol, Advanced Blockchain Dynamic Source Routing (ABCD) \cite{majumder2025} and a decentralized PKI for authentication. Digital Ids Each node in the system are assigned a digital ID along with a set of cryptographic keys which can be used to prove its identity and keep the data exchanges secure.
...
This foundational paper introduces the ABCD protocol \cite{majumder2025}. It is designed to make routing in MANETs more secure. The protocol combines DSR algorithm with blockchain to store route and node information safely. A key feature is the use of homomorphic encryption, which lets nodes process data without decrypting it, keeping the information private. The system builds trust by giving each node a reliability score based on its good and bad actions. Routes that include  less reliable nodes get a higher cost, so the system avoids them.

The main strength of the ABCD protocol \cite{majumder2025} is its smart use of homomorphic encryption and a blockchain-based reputation system. However, it is not easy to use in real-time vehicular networks. Homomorphic encryption needs a lot of computing power, which makes it hard to run on small, low power devices. Also, the use of one single blockchain for all network tasks slows down performance and makes it difficult to scale.
...
The paper's summary integrates blockchain for a trusted routing environment, while deploying deep learning models for proactive traffic prediction and anomaly detection. A BHO algorithm is used, which selects the most stable and secure communication paths. This approach leverages the IPFS for decentralized data storage and a DPBFT consensus mechanism to validate transactions \cite{swamy2024}.\\
...
forwarding takes place between different clusters \cite{guo2023}.\\
...
The regression based technique is used to predict when a link in a MANET will break. It uses a least squares polynomial regression model to estimate link failure time. The method studies how signal strength changes as nodes move apart. It creates a polynomial curve to guess when the signal will become too weak for communication. The results show that it gives more accurate predictions of link failure time \cite{patel2020}.
...
\hline
Majumder et al. \cite{majumder2025} & DSR with blockchain, homomorphic encryption, and reputation system. & Delay reduced by 18.5\%, 89.6\% detection; CPU +30\%. & High cost from homomorphic encryption; poor scalability. \\
\hline
Kudva et al. \cite{kudva2021} & Two-level trust via AODV; RSUs as validators. & 92.4\% PDR, 95\% detection, 21\% less overhead. & Strict trust threshold; low anonymity. \\
\hline
Swamy et al. \cite{swamy2024} & Blockchain + deep learning + BHO optimization. & 97.4\% accuracy, 96.1\% PDR, 42 ms latency. & High computational complexity. \\
\hline
Morin et al. \cite{morin2025} & Lightweight LED cipher, pseudo-identities, group key mgmt. & 1.7 ms enc/dec, 40\% lower energy vs AES-128. & Centralized group manager (single failure point). \\
\hline
Sugumaran et al. \cite{sugumaran2025} & Block-DSD using clustering, ANFIS, ECC, IEHO. & +27\% lifetime, 95.6\% PDR, -30\% energy. & Relies on cluster heads; localized failures. \\
\hline
Hou et al. \cite{hou2023} & DBREMM: double-layer blockchain for trust. & 96.3\% accuracy, 94.7\% detection. & Latency in real-time trust aggregation. \\
\hline
Hiba et al. \cite{kamel2024} & Blockchain + DSR + PKI encryption. & 93.8\% PDR, 31 ms latency. & Vulnerable to collusion attacks. \\
\hline
Mouchfiq et al. \cite{mouchfiq2020} & MPR nodes maintain local blockchains. & 60–70\% reduced ledger load (conceptual). & No simulation; theoretical only. \\
\hline
Guo et al. \cite{guo2023} & RL-based ICRA clustering for UAVs. & 94.8\% PDR, +22\% stability. & RL overhead not ideal for small devices. \\
\hline
Mohindra et al. \cite{mohindra2021} & ECC encryption + AOMDV multipath routing. & 91\% PDR, 86\% detection. & No comparison with newer crypto schemes. \\
\hline
Feng et al. \cite{feng2024} & PBAG: blockchain auth via global commitment. & 14.3 ms delay, 99.2\% success. & Ignores device power consumption. \\
\hline
Agrawal et al. \cite{agrawal2023} & NTRU + Shamir scheme for cluster security. & 35\% faster key exchange; quantum-safe. & Complex architecture for large networks. \\
\hline
Liu et al. \cite{chen2019} & GNL: temporal graph learning via Lasso. & 97.1\% accuracy, 21\% faster runtime. & High computational cost; unsuitable for real-time. \\
\hline
Ghaleb et al. \cite{jari2021} & Direct + indirect trust for blackhole attack detection. & 92.5\% detection, 93.7\% PDR. & High overhead in recommendation sharing. \\
\hline
Patel et al. \cite{patel2020} & Polynomial regression for link failure prediction. & 95.2\% accuracy, -17\% packet loss. & Signal-based prediction unreliable in interference. \\
\hline
\end{tabularx}
\end{table}
...
% ===== Use Case 1: Route Discovery =====
\subsection{Use Case 1: Route Discovery}

\begin{usecase}{Route Discovery}
    \addheading{Actors}
    \begin{itemize}
        \item Source Node
    ...
    The source node initiates a secure route discovery process to find the shortest path to the destination node using Dijkstra's algorithm and blockchain validation.
    
    \addheading{Preconditions}
    \begin{itemize}
        \item Source node is registered in the network with a valid key pair
        \item Network has an established blockchain
    \end{itemize}
    
    \addheading{Basic Flow}
    \begin{enumerate}
        \item The Source Node initiates a route discovery request to send data to a specific destination node.
        \item The system triggers Dijkstra's algorithm to compute the shortest path.
        \item The source node digitally signs the Route Request (RREQ).
        \item The system attaches route and security data to the blockchain.
        \item System broadcasts RREQ to neighbor nodes in the shortest path.
        \item System verifies certificates and signatures at each hop.
        \item Upon reaching destination, RREP is sent back to source.
    \end{enumerate}
    
    \addheading{Alternative Flows}
    \begin{itemize}
        \item \textbf{Verification Failure}: If any node fails verification, the RREQ is dropped and the failure is logged.
        \item \textbf{Time-out}: If no RREP is received within timeout period, the process is retried.
    \end{itemize}
    
    \addheading{Postconditions}
    \begin{itemize}
        \item A secure route is established and recorded in the blockchain
        \item All intermediate nodes have verified the route
    \end{itemize}
\end{usecase}

% ===== Use Case 2: Data Transmission =====
\subsection{Use Case 2: Secure Data Transmission}

\begin{usecase}{Secure Data Transmission}
    \addheading{Actors}
    \begin{itemize}
        \item Source Node
        \item Intermediate Nodes
    ...
    \addheading{Description}
    Secure transmission of encrypted data packets along a verified route with integrity checks at each hop.
    
    \addheading{Preconditions}
    \begin{itemize}
        \item A secure route exists and is recorded in the blockchain
        \item Source has data to send to destination
    \end{itemize}
    
    \addheading{Basic Flow}
    \begin{enumerate}
        \item Source encrypts data with destination's public key
        \item Source signs the packet with its private key
        \item Packet is forwarded along the established route
        \item Each intermediate node verifies the signature
        \item Destination decrypts the packet and verifies the signature
    \end{enumerate}
    
    \addheading{Alternative Flows}
    \begin{itemize}
        \item \textbf{Integrity Check Failure}: If any node detects tampering, packet is dropped
        \item \textbf{Route Break}: If a link fails, route maintenance is triggered
    \end{itemize}
\end{usecase}

% ===== Use Case 3: Node Registration =====
\subsection{Use Case 3: Node Registration}

\begin{usecase}{Node Registration}
    \addheading{Actors}
    \begin{itemize}
        \item New Node
        \item Existing Network Nodes
        \item Certificate Authority
    \end{itemize}
    
    \addheading{Description}
    Process for a new node to join the network and be authenticated.
    
    \addheading{Basic Flow}
    \begin{enumerate}
        \item New node generates key pair
        \item Node requests certificate from CA
        \item Node broadcasts join request with certificate
        \item Network nodes validate certificate
        \item On consensus, node is added to blockchain
    \end{enumerate}
    
    \addheading{Alternative Flows}
    \begin{itemize}
        \item \textbf{Invalid Certificate}: Registration is rejected
        \item \textbf{No Consensus}: Node is not added to network
    \end{itemize}
\end{usecase}

% ===== Use Case 4: Route Maintenance =====
\subsection{Use Case 4: Route Maintenance}

\begin{usecase}{Route Maintenance}
    \addheading{Actors}
    \begin{itemize}
        \item Intermediate Nodes
        \item Source Node
    ...
    \addheading{Description}
    Handles route failures and initiates route rediscovery when needed.
    
    \addheading{Basic Flow}
    \begin{enumerate}
        \item Node detects link failure
        \item Generates RERR (Route Error) message
        \item Sends RERR back to source
        \item Source initiates new route discovery
        \item Updates blockchain with new route
    \end{enumerate}
    
    \addheading{Alternative Flows}
    \begin{itemize}
        \item \textbf{No Alternative Route}: Notify source of unreachable destination
        \item \textbf{Multiple Failures}: Implement backoff algorithm
    \end{itemize}
\end{usecase}
...
This chapter explains the proposed approach and working methodology of the system. It describes the main components of the Data Exchange System and how they interact with each other. The chapter also presents the Blockchain System, Blockchain Structures, and security framework used for secure and efficient node to node communication. It further outlines how trust, authentication, and blockchain are used to ensure reliable data exchange and protection against common network attacks in a PANET environment.
        ...
        \section{PANET Security Framework}
            ...
            Following are some security threats in PANETs:
                \begin{itemize}
                    ...
                    \item \textbf{In} a Wormhole attack, attackers set up hidden shortcut paths that reroute traffic to bad nodes.
        ...
        Following are the important components of the  Security Framework
            ...
            \begin{figure}[htbp]
                \centering
                \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Figures/pan_local_trust_flowchart_colorful.pdf}
      ...
      \textbf{Intermediate Variables: } \\
                ...
                \begin{figure}[htbp]
                    \centering
                    \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Figures/PKCertChain_Node_Registration_Flowchart.pdf}
                    \caption{PKCertChain Node Registration Flowchart}
                    \label{fig:PKCertChain_Flowchart}
                \end{figure}
            ...
            \begin{enumerate}
                ...
                \begin{figure}[htbp]
                    \centering
                    \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Figures/RouteLogChain_Sender_Flowchart-2.pdf}
                    \caption{RouteLogChain Packet Routing and Path Trust. Every node verifies signatures, and packets are re-transmitted if not received. The destination node verifies using Rolling Signature.}
                    \label{fig:RLC}
                \end{figure}
            \FloatBarrier
...
\begin{figure}[htbp]
    \centering
    \includegraphics{Figures/blockchain.PNG}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/SybilDFD2.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/blackhole_attack_DFD.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/wormhole_level2_DFD.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics{Figures/replay_level2_DFD.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics{Figures/simulation_graph.jpg}
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_blackhole.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_wormhole.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics{Figures/pdr_sybil.png}
